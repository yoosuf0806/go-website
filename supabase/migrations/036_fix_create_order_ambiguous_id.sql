-- 036_fix_create_order_ambiguous_id.sql
--
-- Bugfix for create_order() (introduced in 034 / 035): placing an order failed
-- with "column reference \"id\" is ambiguous".
--
-- The function is declared `returns table (id uuid, order_no int)`, so `id` is
-- an output column in scope for the whole body. Pass 2's per-item lookup did
--   select id, price_per_piece, ... from products ...
-- and Postgres couldn't tell that bare `id` from the RETURNS TABLE `id`. The fix
-- is simply to qualify the columns as products.*. No behavioural change.
--
