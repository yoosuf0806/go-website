...
// Shared landing page for /corporate and /wedding: hero banner, trust stats,
// "Everything Handled for You", occasions grid, pricing, and the existing
// flavour-picker + quote-request form. Corporate and wedding each pass in
// their own fully independent `content` + `seo`.
export default function QuoteLandingPage({
  ...
  async function onSubmit(data: QuoteFormRaw) {
    try {
      await mutation.mutateAsync({ ...data, category, pieceCount: Number(data.pieceCount) })
      setSubmitted(true)
      reset()
      setSelectedId(null)
    } catch (err) {
  ...
  return (
    <div>
      ...
      <QuoteHero hero={content.hero} ctaHref={`#${FORM_ANCHOR_ID}`} />
      ...
      <div id={FORM_ANCHOR_ID} className="mx-auto max-w-6xl scroll-mt-24 px-4 py-16">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-2">
          ...
          <div>
            ...
            <h2 className="font-display text-[clamp(1.8rem,3vw,2.5rem)] font-semibold leading-tight text-navy">
              {content.heading}
            </h2>
            <div className="mt-2 flex items-center gap-2 text-sm">
              ...
              <span className="text-neutral-400">
                Loved by 200+ {category === 'wedding' ? 'couples' : 'teams'} across Colombo
              </span>
            </div>

            <p className="mt-3 text-sm leading-relaxed text-neutral-500">{content.intro}</p>
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="mt-6 rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm">
                <div className="flex items-baseline justify-between">
                  <h3 className="font-display text-xl font-semibold">Request a Quote</h3>
...
function SuccessBanner({ onReset }: { onReset: () => void }) {
  return (
    <div className="mt-6 rounded-3xl border border-neutral-200 bg-white p-8 text-center shadow-sm">
      ...
      <h3 className="mt-3 font-display text-xl font-semibold">Quote request received!</h3>
